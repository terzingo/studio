...
const mockJobData: { [key: string]: { beforeImageId: string; afterImageId: string } } = {
  'TAD005': { beforeImageId: 'portfolio-3', afterImageId: 'product-2' },
  'TAD006': { beforeImageId: 'portfolio-6', afterImageId: 'product-4' },
  'default': { beforeImageId: 'portfolio-2', afterImageId: 'product-13' },
};


export default function CompleteJobFlowPage() {
    ...
    const jobImages = mockJobData[jobId] || mockJobData['default'];

    const beforeImage = PlaceHolderImages.find(p => p.id === jobImages.beforeImageId);
    const afterImage = PlaceHolderImages.find(p => p.id === jobImages.afterImageId);
             ...
             <div className="mx-auto grid w-full max-w-4xl gap-4">
                <div className="flex items-center gap-4">
                     <Button variant="outline" size="icon" asChild>
                        <Link href={`/tailor-dashboard/jobs/${jobId}`}>
                            <ArrowRight className="h-4 w-4 scale-x-[-1]" />
                            <span className="sr-only">Geri</span>
                        </Link>
                    </Button>
                    <h1 className="text-xl sm:text-2xl font-semibold">İşi Tamamla: #{jobId}</h1>
                </div>
